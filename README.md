...
含全角分号/冒号等非 ASCII 字符）都会让请求被拒绝。不做静默跳过 ——
写错的规则退化成透传时，上游会收到对外模型名并报 404，
而那与「故意不配映射」在结果上完全同形，无从排查。
写错的片段原文只进看板（`gateway.mapping.invalid`），对客户端只回通用格式
说明：片段可能含真实上游模型名，而 Header 常由 new-api 这类中间层注入，
400 会被原样透传给终端用户 —— 那正是脱敏要挡掉的形态。
...
上面说的是「上游支持时透传」。反过来的情况：**主力模型不支持某项能力**，
比如 DeepSeek-V3 收到带图请求会直接报 400。此时可以声明「这项能力交给谁」，
网关在撞错后自动换过去，而客户端看到的仍是它请求时用的模型名。

```
X-Model-Capability: vision:qwen-vl-max;tools:deepseek-v3;file:qwen-long
```

键用英文，大小写和两侧空格都不敏感；值是上游真实模型名：
...
| 音频理解 | `audio` |
| 视频理解 | `video` |
| 工具调用 | `tools` / `tool` / `function` |
| 文档理解 | `file` / `document` / `doc` |

无法识别的键会被整段丢弃，不会退化成默认能力——配置写错时宁可不生效，
也不要静默路由到一个没打算用的模型。

也可以用环境变量配全局默认，请求头优先：

```bash
MODEL_CAPABILITY=vision:qwen-vl-max;file:qwen-long
```

**触发时机：先撞错，再切换。** 网关不预判模型能力 —— 上游明确报错才说明
当前模型确实不支持，这样不会为了假设中的不支持而牺牲主力模型的效果。
能力切换的重试状态集比通用兜底更宽，额外收录 400 / 404 / 413 / 415 / 422，
因为「模型不支持某能力」的报错恰好落在这里。仍然排除 401 / 403：
凭据无效换任何模型都过不去。

**文档理解是唯一的例外**，请求体一旦出现 `{"type":"file","file":{"file_id":"…"}}`
就直接改走文档模型，不等报错。两个原因：`file_id` 由上游文件服务颁发并绑定
特定模型，发给不支持的模型往往返回 "file not found" 这类无法与「文件真的不存在」
区分的误导性错误；而且文件请求体积大，撞一次错的成本远高于普通请求。

请求同时用到多项能力时（带图 + 带工具），按 识图 → 音频 → 视频 → 工具 → 文档
的顺序取第一个命中的。多模态排在工具之前，因为前者是硬约束 —— 模型不支持
就根本无法处理这个输入。

能力切换优先于 `MODEL_MAP_FALLBACK`：它知道缺的是什么能力，命中率明显更高。
沿用同一套安全边界 —— 正文开始下发后不重试、每个请求最多切一次、
目标与当前模型相同时不切。日志里 `caps` 是识别到的能力，`cap_route` 是实际
切换到的那一项。
...
**不要在网关里做格式转换或参数校验。** 一旦网关开始改写请求体结构，
每次上游更新协议都要跟着改代码，而这恰好是本项目要避免的耦合。

**不要维护一份「模型能力矩阵」。** 上面的能力切换只做两件很轻的事：
按几个固定的 JSON 结构判断「这个请求用到了什么」，以及在上游报错后换个模型名。
它不判断「某个模型支持什么」—— 那份表格永远滞后于上游，且一旦写错就会
把本可以成功的请求路由到错误的模型上。谁支持什么由使用者显式声明，
网关只负责在真的失败时执行切换。

**没有配置能力头时，行为与之前完全一致** —— 不解析请求体、不做能力识别，
上游报错原样透传给客户端。识别只在声明了目标模型的能力上进行，
没有切换目标就没有识别的意义（见 `TestDetectOnlyScansWantedCapabilities`）。
